...
export default function BrainGymMVP() {
  ...
  const updateRecovery = useCallback((patch) => setState((s) => ({ ...s, recovery: { ...s.recovery, ...patch, updatedAt: nowIso() } })), []);
  ...
  const addWorkoutFromTemplate = (templateId) => {
    ...
    const workout = {
      ...
      sessionRpe: t.exercises.length ? Math.round(t.exercises.reduce((a, e) => a + (e.rpe || 7), 0) / t.exercises.length) : 7,
  ...
  const addCard = (deckId, front, back) => {
    ...
    const b = (back ?? "").trim();
    if (!f || !b) {
      pushNotification({ type: "tip", text: "Carte non ajoutée : recto et verso sont obligatoires." });
      return;
    }
  ...
  const suggestedDeckId = useMemo(
    () => state.profile.preferredStudyDeckId || (state.decks[0]?.id ?? ""),
    [state.profile.preferredStudyDeckId, state.decks]
  );
